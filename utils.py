...
def loss_with_l2_regularization(nima_result, filters, gamma=config.gamma, initial_filters=None):
    ...
    if initial_filters is not None:
        ...
        print_msg("Current Filters: {}".format([filters[x].item() for x in range(13)]), 3)
...
#? 直近nエポックに最大スコアが含まれるとTrue, でなければFalse
def monitor_enhance_progress(score_list, patience=10):
    maxscore = max(score_list)
    recent_score_list = []
    if len(score_list) > patience:
        for i in range(1, patience):
            recent_score_list.append(score_list[-i])
        if maxscore in recent_score_list:
            return True
        else:
            return False
    else:
